#!/usr/bin/env python3
"""
Blog Post Image Automation Script

This script:
1. Downloads appropriate images from Unsplash for a blog post
2. Resizes them to recommended dimensions
3. Saves them to the respective directories
4. Updates the front matter with image credits
...
GIT_COMMIT_MESSAGE_TEMPLATE = "Add images for blog post: {}"

def setup_argparse():
    """Set up command-line argument parsing."""
    parser = argparse.ArgumentParser(description='Automate blog post image setup')
    parser.add_argument('post_path', help='Path to the blog post markdown file')
    parser.add_argument('--search', default='code programming ai technology', 
                        help='Search terms for Unsplash (default: "code programming ai technology")')
...
def search_unsplash(search_terms, api_key):
    """Search Unsplash for images matching the search terms."""
    if not api_key:
    ...
    try:
        ...
        return {
            ...
            'credit_url': image['user']['links']['html'],
            'id': image['id']
        }
...
def update_front_matter(post_info, image_paths, image_credit):
...
def main():
    ...
    print(f"Searching Unsplash for: {args.search}")
    image_info = search_unsplash(args.search, unsplash_api_key)
    ...
    update_front_matter(post_info, image_paths, image_info['credit'])
    
    # Git operations
    if not args.no_commit:
        print("Performing Git operations...")
        git_operations(post_info, full_image_paths)
    
    print(f"\nImage setup complete for post: {post_info['title']}")
